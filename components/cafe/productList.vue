<template>
  <div>
    <div v-for="(cat, i) in menu" :key="cat.name" class="product-list-wrapper">
      <div v-if="activeCategory == i || filtered" :key="cat.pk" class="product-list">
        <div
...
export default {
  ...
  methods: {
    countChange(index, count, product) {
      ...
      if (product.count + count < product.reduceLimit) {
        this.toaster(
    ...
    menu: {
      ...
      handler(newValue, oldValue) {
        if (newValue.length > 0) {
          ...
          if (this.filtered) this.activeProducts = this.menu[0].products
          else this.activeProducts = this.menu[this.activeCategory].products
